...
object KMeansRunner {
  ...
  def apply(records: RDD[ArchiveRecord], sc: SparkContext) = {
    ...
    val lemmatized = records.keepValidPages().mapPartitions(r => {
      ...
      r.map(q => getLemmas(Jsoup.parse(q.getContentString).body().text(), pipeline, stopwords))
    })
    lemmatized.foreach(println)
    
  }

  def getLemmas(text: String, pipeline: StanfordCoreNLP, stopWords: Broadcast[Set[String]]): Seq[String] = {
