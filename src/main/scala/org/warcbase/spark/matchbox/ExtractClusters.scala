...
object ExtractClusters {
  def apply(records: RDD[ArchiveRecord], sc: SparkContext, k: Int = 20, numIterations: Int = 20, minDocThreshold: Int=5) = {
    ...
    //sc.getConf.setExecutorEnv("spark.kryoserializer.buffer.max", "20G")
    // sc.getConf.setExecutorEnv("spark.kryoserializer.buffer.max.mb", "32768")
    ...
    new KMeansArchiveCluster(KMeans.train(tfidf, k, numIterations), tfidf, lemmatized, sc, rec)
  }
